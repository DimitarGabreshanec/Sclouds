...
class AddPhotoViewController: UIViewController, UINavigationControllerDelegate {
    ...
    @IBAction func nextButton(_ sender: Any) {
        ...
        //self.dataImage = UIImagePNGRepresentation(selectedImageView.image!)
        //        let dataImage = selectedImageView.pngData() as NSData?
        if selectedImageView.image != nil {
            SwiftLoader.show(title:"Please Wait...", animated: false)
            // Marks Api Calling for uplaod images from form-data
            apiCalingUpload(songData: dataImage as! Data, fileName: "File")
    ...
    //PSApi.upload(.photo, params: generalInfoParam as [String : AnyObject], isShowAlert: true, controller: self, isNeedToken: true, fileName: "file", binary: dataImage) { (response) in
       //   }
        }
        else{
            Utility.SubmitAlertView(viewController: self, title: "Alert!", message: "Please select profile image")
    ...
    func apiCalingUpload(songData:Data?, fileName:String) {
            ...
            Alamofire.upload(multipartFormData: { multipartFormData in
                multipartFormData.append(songData!, withName: "fileset",fileName: "file", mimeType: "image/jpg")
//                for (key, value) in parameters {
//                    multipartFormData.append(value.data(using: String.Encoding.utf8)!, withName: key)
//                } //Optional for extra parameters
            },to:photoUpload)  { (result) in
                switch result {
...
extension AddPhotoViewController: UIImagePickerControllerDelegate{
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]){
        ...
        self.dataImage = selectedImage.pngData() as! NSData
        print("\(dataImage)")
        print("\(selectedImage)")
