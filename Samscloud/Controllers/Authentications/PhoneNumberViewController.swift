...
class PhoneNumberViewController: UIViewController ,UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        if isFromChangeNumber {
            phoneNumberTextField.text = DefaultManager().getPhoneNumber() ?? ""
        }
    }
    ...
    func phoneNumberAPiCaling() {
        ...
        let mobileNo = phoneNo!.replacingOccurrences(of: "-", with: "")
        //phoneNumberTextField.text = "\(self.areaCodeLabel.text ?? "+1")" + mobileNo
        
        let email = DefaultManager().getEmail()  ?? ""
        
        let param = ["email" : email, "mobile_number":"\(self.areaCodeLabel.text ?? "+1")" + mobileNo]
        
        PSApi.apiRequestWithEndPointSome(.phone, params: param as [String : AnyObject], isShowAlert: true, controller: self, isNeedToken: false) { (response) in
            //kAppDelegate.tokenAuth = userDefault.object(forKey:"token") as! String
            SwiftLoader.hide()
            ...
            if  statusCode == 200 {
                ...
                let otpVC = storyBoard.instantiateViewController(withIdentifier: "VerifiCodeVC") as! VerifiCodeViewController
                otpVC.phoneNumber = "\(self.areaCodeLabel.text ?? "+1")" + mobileNo
                otpVC.isChangePassword = self.isFromChangeNumber
                otpVC.senderVC = self.senderVC
...
//        let mobileNo = phoneNo!.replacingOccurrences(of: "-", with: "")
//
//        //let param = ["username":emailTextField.text!,]
//
//        let param = ["mobile_number": mobileNo] as [String : Any]
//          PSApi.apiRequestWithEndPointSome(.phone, params: param as [String : AnyObject], isShowAlert: true, controller: self, isNeedToken: false) { (response) in
//            print(response)
//            let statusCode = response.response?.statusCode
//            if  statusCode == 200 {
