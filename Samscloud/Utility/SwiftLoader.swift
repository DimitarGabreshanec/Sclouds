...
public class SwiftLoader: UIView {
    ...
    // MARK: - STRUCT (LOADER CONFIGURATION)
    
    public struct Config {
        
        public var size: CGFloat = 120.0 // Size of loader
        public var spinnerColor = UIColor.white
        public var spinnerLineWidth: Float = 1.0
        public var titleTextColor = UIColor.white
        public var speed: Int = 1 // spinner speed
        public var titleTextFont: UIFont = UIFont.boldSystemFont(ofSize: 16.0) // Font for title in loader
        public var backgroundColor = Utility.getThemeColor() // Loader bg color
        public var foregroundColor = UIColor.clear
        public var foregroundAlpha: CGFloat = 0.0
        public var cornerRadius: CGFloat = 10.0 // for loader
    ...
    class var sharedInstance: SwiftLoader {
        struct Singleton {
            static let instance = SwiftLoader(frame: CGRect(origin: CGPoint(x: 0, y: 0),
                                                            size: CGSize(width: Config().size, height: Config().size)))
        }
    ...
    public class func setConfig(config: Config) {
        ...
        loader.frame = CGRect(origin: CGPoint(x: 0, y: 0),
                              size: CGSize(width: loader.config.size, height: loader.config.size))
    }
    ...
    private func update() {
        DispatchQueue.main.async {
            ...
            if (self.titleLabel == nil) {
                self.titleLabel = UILabel(frame: CGRect(origin: CGPoint(x: loaderTitleMargin, y: loaderSpinnerMarginTop + loadingViewSize),
                ...
                self.titleLabel?.frame = CGRect(origin: CGPoint(x: loaderTitleMargin, y: loaderSpinnerMarginTop + loadingViewSize),
                                                size: CGSize(width: self.frame.width - loaderTitleMargin * 2, height: 42.0))
            }
    ...
    func frameForSpinner() -> CGRect {
        ...
        if (self.title == nil) {
            ...
            return CGRect(origin: CGPoint(x: loaderSpinnerMarginSide, y: yOffset),
                          size: CGSize(width: loadingViewSize, height: loadingViewSize))
        }
        return CGRect(origin: CGPoint(x: loaderSpinnerMarginSide, y: loaderSpinnerMarginTop),
                      size: CGSize(width: loadingViewSize, height: loadingViewSize))
    }
