...
public class InAppNotify{
    ...
    public class func Show(_ announcement: Announcement, to: UIViewController) {
        ...
        DispatchQueue.main.async {
            ...
            self.currentNotify?.hide(false)
            //Create new notify
            self.currentNotify = NotificationFactory()
            //Build new notify
            self.currentNotify?.build(forAnnouncement : announcement, to: to)
        }
...
public struct NotificationSize {
    ...
    public static let imageOffset       : CGFloat = 16
    public static var height            : CGFloat = UIApplication.shared.isStatusBarHidden ? 80*widthFactor : 80*widthFactor
    public static var textOffset        : CGFloat = 65 + 10
}
